...
public class Arquivo
{
    ...
    public void inserirAux(int num)
    {
        ...
        seekArq(filesizeAux());//ultimo byte
        reg.gravaNoArq(aux);
    }
    ...
    public void shake_sort()
    {
        ...
        for (int inicio = 0, fim = tl - 1; inicio < fim; inicio++)
        {
            for (int i = inicio; i < fim; i++) {
                seekArq(i); vet1.leDoArq(arquivo);
                seekArq(i + 1); vet2.leDoArq(arquivo);
                compProg++;
        ...
        for (dist = 4; dist > 0; dist /= 2){
            for (i = 0; i < dist; i++){
                for (j = i; j + dist < tl; j+=dist){
                    seekArq(j); vet1.leDoArq(arquivo);
                    seekArq(j + dist); vet2.leDoArq(arquivo);
                    compProg++;
                    if(vet1.getCodigo() > vet2.getCodigo()){
                        movProg+=2;
                        swap(j, j + dist);
                        k = j;
                        seekArq(k); vet1.leDoArq(arquivo);
                        seekArq(k - dist); vet2.leDoArq(arquivo);
                        compProg++;
